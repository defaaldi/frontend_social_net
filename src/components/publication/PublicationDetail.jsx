...
export const PublicationDetail = () => {
  ...
  const getPublication = async () => {
    try {
      ...
      const request = await fetch(
        `${Global.url}publication/show-publication/${id}`,
        {
          method: "GET",
          headers: {
  ...
  const userImage =
    user.image && user.image !== "default.png" ? user.image : avatar; // Imagen predeterminada si no hay imagen
